...
docker_setup_db() {
	...
	dbAlreadyExists="$(
		POSTGRES_DB= docker_process_sql --dbname postgres --set db="$BABELFISH_DB" --tuples-only <<-'EOSQL'
			SELECT 1 FROM pg_database WHERE datname = :'db' ;
		EOSQL
	)"
	if [ -z "$dbAlreadyExists" ]; then
		POSTGRES_DB= docker_process_sql --dbname postgres \
			--set bbfdb="$BABELFISH_DB" \
			--set bbfuser="$BABELFISH_USER" \
...
# create initial database
# uses environment variables for input: BABELFISH_DB
docker_setup_babelfish_db() {
	echo "Setting babelfish migration_mode to $BABELFISH_MIGRATION_MODE on DB $BABELFISH_DB"
	BABELFISH_POSTGRES_DB= docker_process_sql --dbname "$BABELFISH_DB" \
	--set bbfdb=$BABELFISH_DB \
	--set mm=$BABELFISH_MIGRATION_MODE \
	--set user=$BABELFISH_USER <<-'EOSQL'
		CREATE EXTENSION IF NOT EXISTS "babelfishpg_tds" CASCADE;
		ALTER SYSTEM SET babelfishpg_tsql.database_name = :"bbfdb";
		ALTER DATABASE :bbfdb SET babelfishpg_tsql.migration_mode = :'mm';
